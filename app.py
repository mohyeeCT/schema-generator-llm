...
def extract_comprehensive_business_info(soup: BeautifulSoup) -> Dict[str, Any]:
    ...
    name_selectors = [
        '[itemprop="name"]', 'h1', '.company-name', '.business-name',
        '[class*="name"]', '.logo-text'
    ]
    ...
    if not url:
        ...
        with st.spinner("Extracting comprehensive content..."):
            try:
                ...
                with st.spinner("Generating comprehensive schema..."):
                    ...
                    if schema_data:
                        ...
                        if "contactPoint" in schema_data and len(schema_data["contactPoint"]) > 1:
                            ...
                            enhancements.append(f"{dept_count} departments with specialized contact info")
                        if "sameAs" in schema_data and len(schema_data["sameAs"]) > 0:
                            social_count = len(schema_data["sameAs"])
                            enhancements.append(f"{social_count} social platforms linked")
                        if "subjectOf" in schema_data:
                            wiki_count = len(schema_data["subjectOf"])
                            enhancements.append(f"{wiki_count} topic links for authority")
                        if "knowsAbout" in schema_data:
                            expertise_count = len(schema_data["knowsAbout"])
                            enhancements.append(f"{expertise_count} knowledge domains defined")
                        if "keywords" in schema_data and len(schema_data["keywords"]) > 5:
                            keyword_count = len(schema_data["keywords"])
                            enhancements.append(f"{keyword_count} industry-specific terms")
                        if "location" in schema_data:
                            enhancements.append("Location with map data included")
                        for enhancement in enhancements:
                            st.success(enhancement)
                        if not enhancements:
                            st.info("Consider using Organization or EntitySchema template for maximum features")
                ...
                st.info("Ensure the URL is accessible and contains valid HTML content.")
